...
function renderFilesList(files: FileRecord[]) {
  ...
  return `
    <ul class="files-list">
      ${files
        .map((file) => {
          const isUploader = currentUser && currentUser.id === file.uploaded_by_id
          const isProfessor = currentUser && currentUser.rol === 'Profesor'
          const showEdit = !!isUploader
          const showDelete = !!(isUploader || isProfessor)
          return `
            <li class="file-item" data-file-id="${file.id}">
              ...
              <div class="file-actions">
                ...
                ${showEdit ? `<button class="button small outline edit-file-btn" data-file-id="${file.id}">Editar</button>` : ''}
                ${showDelete ? `<button class="button small danger delete-file-btn" data-file-id="${file.id}">Eliminar</button>` : ''}
              </div>
            </li>
          `
        })
...
export async function renderClassroomsPage(root: HTMLElement) {
  ...
  if (sidebarNav) {
    sidebarNav.innerHTML = `
      ...
      ${currentUser.rol === 'Admin' ? '<a href="#/admin/professor-validations" data-router class="nav-link">Solicitudes</a>' : ''}
    `
  }
